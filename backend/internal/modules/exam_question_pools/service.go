...
type Service interface {
	...
	Create(context.Context, uuid.UUID, CreateExamQuestionPoolRequest) (shared.Record, error)
	Update(context.Context, uuid.UUID, uuid.UUID, UpdateExamQuestionPoolRequest) (shared.Record, error)
...
func (s *service) Create(ctx context.Context, tenantID uuid.UUID, req CreateExamQuestionPoolRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		...
		_ = s.deps.Rabbit.Publish(ctx, "exam_question_pools.created", body)
	}
	return out, err
}
func (s *service) Update(ctx context.Context, tenantID, id uuid.UUID, req UpdateExamQuestionPoolRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		body, _ := json.Marshal(out)
		_ = s.deps.Rabbit.Publish(ctx, "exam_question_pools.updated", body)
	}
	return out, err
}
func (s *service) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	...
	if err == nil && s.deps.Rabbit != nil {
		_ = s.deps.Rabbit.Publish(ctx, "exam_question_pools.deleted", []byte(id.String()))
	}
	return err
}
