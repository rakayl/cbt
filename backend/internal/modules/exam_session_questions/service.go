...
type Service interface {
	...
	Create(context.Context, uuid.UUID, CreateExamSessionQuestionRequest) (shared.Record, error)
	Update(context.Context, uuid.UUID, uuid.UUID, UpdateExamSessionQuestionRequest) (shared.Record, error)
...
func (s *service) Create(ctx context.Context, tenantID uuid.UUID, req CreateExamSessionQuestionRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		...
		_ = s.deps.Rabbit.Publish(ctx, "exam_session_questions.created", body)
	}
	return out, err
}
func (s *service) Update(ctx context.Context, tenantID, id uuid.UUID, req UpdateExamSessionQuestionRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		body, _ := json.Marshal(out)
		_ = s.deps.Rabbit.Publish(ctx, "exam_session_questions.updated", body)
	}
	return out, err
}
func (s *service) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	...
	if err == nil && s.deps.Rabbit != nil {
		_ = s.deps.Rabbit.Publish(ctx, "exam_session_questions.deleted", []byte(id.String()))
	}
	return err
}
