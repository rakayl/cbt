...
func (h Handler) StudentHistory(c *fiber.Ctx) error {
	q := pagination.New(c.Query("page"), c.Query("limit"), c.Query("search"), c.Query("sort"))
	out, err := h.service.StudentHistory(c.Context(), shared.TenantID(c), shared.UserID(c), q)
	if err != nil {
		return response.Error(c, fiber.StatusUnprocessableEntity, "cannot load student exam history", err)
	}
	return response.OK(c, out)
}

func (h Handler) StudentResultDetail(c *fiber.Ctx) error {
	sessionID, err := uuid.Parse(c.Params("session_id"))
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, "invalid session id", nil)
	}
	out, err := h.service.StudentResultDetail(c.Context(), shared.TenantID(c), shared.UserID(c), sessionID)
	if err != nil {
		return response.Error(c, fiber.StatusUnprocessableEntity, "cannot load student exam result", err)
	}
	return response.OK(c, out)
}
...
func (h Handler) Update(c *fiber.Ctx) error {
	...
	var req UpdateExamSessionRequest
	if err := c.BodyParser(&req); err != nil {
		return response.Error(c, fiber.StatusBadRequest, "invalid payload", err)
	}
