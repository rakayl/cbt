...
func RegisterRoutes(api fiber.Router, deps shared.Deps) {
	...
	group.Get("/:id/rankings", middleware.RequirePermission(PermissionRead), h.Rankings)
	group.Get("/:id/invites", middleware.RequirePermission("exams:invite"), h.ListInvites)
	group.Get("/:id/invite-roster", middleware.RequirePermission("exams:invite"), h.InviteRoster)
	...
	group.Put("/:id/access-status", middleware.RequirePermission(PermissionWrite), h.UpdateAccessStatus)
