...
type Service interface {
	...
	Create(context.Context, uuid.UUID, CreateFaceDetectionLogRequest) (shared.Record, error)
	Update(context.Context, uuid.UUID, uuid.UUID, UpdateFaceDetectionLogRequest) (shared.Record, error)
...
func (s *service) Create(ctx context.Context, tenantID uuid.UUID, req CreateFaceDetectionLogRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		...
		_ = s.deps.Rabbit.Publish(ctx, "face_detection_logs.created", body)
	}
	return out, err
}
func (s *service) Update(ctx context.Context, tenantID, id uuid.UUID, req UpdateFaceDetectionLogRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		body, _ := json.Marshal(out)
		_ = s.deps.Rabbit.Publish(ctx, "face_detection_logs.updated", body)
	}
	return out, err
}
func (s *service) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	...
	if err == nil && s.deps.Rabbit != nil {
		_ = s.deps.Rabbit.Publish(ctx, "face_detection_logs.deleted", []byte(id.String()))
	}
	return err
}
