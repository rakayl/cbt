...
type Service interface {
	...
	Create(context.Context, uuid.UUID, CreateLecturerRequest) (shared.Record, error)
	Update(context.Context, uuid.UUID, uuid.UUID, UpdateLecturerRequest) (shared.Record, error)
...
func (s *service) Create(ctx context.Context, tenantID uuid.UUID, req CreateLecturerRequest) (shared.Record, error) {
	...
	err = tx.QueryRow(ctx, `
		INSERT INTO lecturers(id, tenant_id, code, name, description, status, metadata, user_id)
	...
	if err == nil && s.deps.Rabbit != nil {
		...
		_ = s.deps.Rabbit.Publish(ctx, "lecturers.created", body)
	}
	return out, err
}
func (s *service) Update(ctx context.Context, tenantID, id uuid.UUID, req UpdateLecturerRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		body, _ := json.Marshal(out)
		_ = s.deps.Rabbit.Publish(ctx, "lecturers.updated", body)
	}
	return out, err
}
func (s *service) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	...
	if err == nil && s.deps.Rabbit != nil {
		_ = s.deps.Rabbit.Publish(ctx, "lecturers.deleted", []byte(id.String()))
	}
	return err
}
