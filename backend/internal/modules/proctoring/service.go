...
type Service interface {
	...
	Create(context.Context, uuid.UUID, CreateProctoringRequest) (shared.Record, error)
	Update(context.Context, uuid.UUID, uuid.UUID, UpdateProctoringRequest) (shared.Record, error)
	Delete(context.Context, uuid.UUID, uuid.UUID) error
	SessionTimeline(context.Context, uuid.UUID, uuid.UUID) (ProctoringTimelineResponse, error)
}
type service struct {
	repo Repository
	deps shared.Deps
...
func (s *service) Create(ctx context.Context, tenantID uuid.UUID, req CreateProctoringRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		...
		_ = s.deps.Rabbit.Publish(ctx, "proctoring.created", body)
	}
	return out, err
}
func (s *service) Update(ctx context.Context, tenantID, id uuid.UUID, req UpdateProctoringRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		body, _ := json.Marshal(out)
		_ = s.deps.Rabbit.Publish(ctx, "proctoring.updated", body)
	}
	return out, err
}
func (s *service) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	...
	if err == nil && s.deps.Rabbit != nil {
		_ = s.deps.Rabbit.Publish(ctx, "proctoring.deleted", []byte(id.String()))
	}
	return err
}
