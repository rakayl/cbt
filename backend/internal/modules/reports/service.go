...
type Service interface {
	...
	Create(context.Context, uuid.UUID, CreateReportRequest) (shared.Record, error)
	Update(context.Context, uuid.UUID, uuid.UUID, UpdateReportRequest) (shared.Record, error)
...
func (s *service) Create(ctx context.Context, tenantID uuid.UUID, req CreateReportRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		...
		_ = s.deps.Rabbit.Publish(ctx, "reports.created", body)
	}
	return out, err
}
func (s *service) Update(ctx context.Context, tenantID, id uuid.UUID, req UpdateReportRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		body, _ := json.Marshal(out)
		_ = s.deps.Rabbit.Publish(ctx, "reports.updated", body)
	}
	return out, err
}
func (s *service) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	...
	if err == nil && s.deps.Rabbit != nil {
		_ = s.deps.Rabbit.Publish(ctx, "reports.deleted", []byte(id.String()))
	}
	return err
}
