...
func RegisterRoutes(api fiber.Router, deps shared.Deps) {
	...
	h := NewHandler(svc)
	group := api.Group("/roles", middleware.JWT(deps.Config), ModuleMiddleware())
	...
	group.Get("/:id/permissions", middleware.RequirePermission(PermissionRead), h.Permissions)
	group.Put("/:id/permissions", middleware.RequirePermission(PermissionWrite), h.SetPermissions)
