...
type Service interface {
	...
	Create(context.Context, uuid.UUID, CreateScreenRecordingRequest) (shared.Record, error)
	Update(context.Context, uuid.UUID, uuid.UUID, UpdateScreenRecordingRequest) (shared.Record, error)
...
func (s *service) Create(ctx context.Context, tenantID uuid.UUID, req CreateScreenRecordingRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		...
		_ = s.deps.Rabbit.Publish(ctx, "screen_recordings.created", body)
	}
	return out, err
}
func (s *service) Update(ctx context.Context, tenantID, id uuid.UUID, req UpdateScreenRecordingRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		body, _ := json.Marshal(out)
		_ = s.deps.Rabbit.Publish(ctx, "screen_recordings.updated", body)
	}
	return out, err
}
func (s *service) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	...
	if err == nil && s.deps.Rabbit != nil {
		_ = s.deps.Rabbit.Publish(ctx, "screen_recordings.deleted", []byte(id.String()))
	}
	return err
}
