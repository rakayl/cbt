...
type Service interface {
	...
	Create(context.Context, uuid.UUID, CreateSubscriptionPlanRequest) (shared.Record, error)
	Update(context.Context, uuid.UUID, uuid.UUID, UpdateSubscriptionPlanRequest) (shared.Record, error)
...
func (s *service) Create(ctx context.Context, tenantID uuid.UUID, req CreateSubscriptionPlanRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		...
		_ = s.deps.Rabbit.Publish(ctx, "subscription_plans.created", body)
	}
	return out, err
}
func (s *service) Update(ctx context.Context, tenantID, id uuid.UUID, req UpdateSubscriptionPlanRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		body, _ := json.Marshal(out)
		_ = s.deps.Rabbit.Publish(ctx, "subscription_plans.updated", body)
	}
	return out, err
}
func (s *service) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	...
	if err == nil && s.deps.Rabbit != nil {
		_ = s.deps.Rabbit.Publish(ctx, "subscription_plans.deleted", []byte(id.String()))
	}
	return err
}
