...
type Service interface {
	...
	Create(context.Context, uuid.UUID, CreateTenantRequest) (shared.Record, error)
	Update(context.Context, uuid.UUID, uuid.UUID, UpdateTenantRequest) (shared.Record, error)
...
func (s *service) Create(ctx context.Context, tenantID uuid.UUID, req CreateTenantRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		...
		_ = s.deps.Rabbit.Publish(ctx, "tenants.created", body)
	}
	return out, err
}
func (s *service) Update(ctx context.Context, tenantID, id uuid.UUID, req UpdateTenantRequest) (shared.Record, error) {
	...
	if err == nil && s.deps.Rabbit != nil {
		body, _ := json.Marshal(out)
		_ = s.deps.Rabbit.Publish(ctx, "tenants.updated", body)
	}
	return out, err
}
func (s *service) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	...
	if err == nil && s.deps.Rabbit != nil {
		_ = s.deps.Rabbit.Publish(ctx, "tenants.deleted", []byte(id.String()))
	}
	return err
}
